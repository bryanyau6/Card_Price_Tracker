...
try:
    ...
    with sync_playwright() as p:
        ...
        print(f"\n✅ 情報處理完畢。準備新增 {len(new_cards_to_add)} 張新 VG 卡牌，記錄 {len(price_history_to_add)} 條 VG 價格情報 (JPY)。")
        ...
        if price_history_to_add:
            print(f"     -> 正在將 {len(price_history_to_add)} 條 VG 價格情報 (JPY-Only) **追加**到 `Price_History`...")
